...
function createShareQueue({ repo, storage, publisher, log = () => {} }) {
  ...
  // O totem pode ter fotografado dias antes de o Blob ser configurado.
  // Essas fotos não falharam: foram ignoradas por falta de destino. Ao
  // surgir um publisher real, entram novamente na fila automaticamente.
  if (publisher.configured) repo.retrySkippedShares();
  ...
  return {
    ...
    start() {
      ...
      /* Uma fila que falha em TODO tick não está publicando nada — está
         só enchendo o log. Acontece, por exemplo, quando o banco fecha
         embaixo dela. Depois de algumas falhas seguidas ela se desliga e
         diz isso uma vez, em vez de repetir o mesmo erro para sempre. */
      let falhasSeguidas = 0;
      const LIMITE_FALHAS = 5;

      timer = setInterval(() => {
        tick()
          .then(() => { falhasSeguidas = 0; })
          .catch(err => {
            falhasSeguidas++;
            lastError = err.message;
            if (falhasSeguidas < LIMITE_FALHAS) {
              log('error', 'fila de publicação falhou neste ciclo', { error: err.message, falhasSeguidas });
              return;
            }
            log('error', 'fila de publicação desligada após falhas seguidas', {
              error: err.message,
              falhasSeguidas,
              acao: 'as fotos continuam salvas; use POST /api/share/retry após corrigir',
            });
            clearInterval(timer);
            timer = null;
          });
