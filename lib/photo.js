...
async function loadFrame(session, aspectRatio) {
  ...
  if (frame) {
    try {
      ...
      console.error('Moldura da sessão ilegível, tentando a padrão:', err.message);
    }
  }

  /* Moldura padrão do projeto.
     É o que dispensa carregar a arte a cada evento — e o que impede a
     falha mais cara possível: uma noite inteira de fotos sem moldura
     porque ninguém lembrou de subir o arquivo. */
  return await moldurraPadrao(aspectRatio);
}

/* Cache por proporção: a arte não muda durante o evento, e ler um PNG
   grande a cada foto seria desperdício. */
const padraoEmCache = new Map();

/**
 * O arquivo de moldura que corresponde à proporção pedida.
 *
 * A ordem reflete o quanto cada candidato é específico: primeiro a arte
 * daquela orientação, depois a genérica. Assim um totem com duas artes
 * troca sozinho quando o operador gira a proporção, e um totem com uma
 * só continua funcionando como antes.
 */
function candidatosDeMoldura(aspectRatio) {
  const nomes = aspectRatio === '4:3'
    ? ['horizontal.png', 'deitado.png']
    : ['vertical.png', 'em-pe.png'];

  const caminhos = nomes.map(n => path.join(config.framesDir, n));
  caminhos.push(path.join(config.framesDir, 'padrao.png'));

  // DEFAULT_FRAME continua tendo a última palavra, para quem aponta
  // para uma arte fora do projeto.
  if (config.defaultFramePath) caminhos.unshift(config.defaultFramePath);
  return caminhos;
}

async function moldurraPadrao(aspectRatio = '3:4') {
  const chave = aspectRatio === '4:3' ? '4:3' : '3:4';
  if (padraoEmCache.has(chave)) return padraoEmCache.get(chave);

  for (const caminho of candidatosDeMoldura(chave)) {
    try {
      const bytes = await fs.promises.readFile(caminho);
      console.log(`Moldura ${chave} carregada de ${caminho}`);
      padraoEmCache.set(chave, bytes);
      return bytes;
    } catch { /* próximo candidato */ }
  }

  // Ausência é legítima: um totem sem arte configurada fotografa igual.
  padraoEmCache.set(chave, null);
  return null;
}

/** Esvazia o cache — usado quando a arte é trocada em disco. */
function esquecerMolduras() {
  padraoEmCache.clear();
}
...
async function composeFinalPhoto(input, options = {}) {
  ...
  /* Cópia do ORIGINAL num segundo disco.
     É o que substitui o cartão como redundância quando a câmera roda em
     "PC Only". Guarda o original, e não o master: dele se reconstrói
     qualquer versão, e é o único arquivo que não dá para refazer.

     Fora do await de propósito — um HD externo lento não pode atrasar a
     resposta ao convidado, e falhar aqui não invalida a foto que já
     está salva. */
  if (config.backupDir) {
    fs.promises
      .mkdir(config.backupDir, { recursive: true })
      .then(() => fs.promises.writeFile(path.join(config.backupDir, `${stamp}_original.jpg`), input))
      .catch(err => console.error('Cópia de segurança falhou (a foto está salva):', err.message));
  }
...
module.exports = { composeFinalPhoto, centerCrop, ratioValue, detectarJanela, loadFrame, esquecerMolduras };
