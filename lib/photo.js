...
async function loadFrame(session, aspectRatio) {
  ...
  return await moldurraPadrao();
}

let padraoEmCache;

async function moldurraPadrao() {
  if (padraoEmCache !== undefined) return padraoEmCache;

  try {
    padraoEmCache = await fs.promises.readFile(config.defaultFramePath);
    console.log(`Moldura padrão carregada de ${config.defaultFramePath}`);
  } catch {
...
module.exports = { composeFinalPhoto, centerCrop, ratioValue, detectarJanela, loadFrame };
