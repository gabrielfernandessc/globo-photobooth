...
function createGphotoCamera({
  ...
  function mudarEstado(novo, detalhe = {}) {
    ...
    for (const ouvir of ouvintes) {
      try { ouvir({ estado: novo, anterior, modelo, erro: ultimoErro, ...detalhe }); } catch { /* ouvinte ruim não derruba a câmera */ }
    }
  }
  ...
  async function detectar() {
    ...
    return linhas[0].replace(/\s{2,}usb:.*$/, '').trim();
  }
  ...
  function iniciarLiveView() {
    if (processoLive || !modelo) return;

    let buffer = Buffer.alloc(0);
    let erroSaida = '';

    processoLive = spawn(binario, ['--camera', modelo, '--port', 'usb:', '--capture-movie', '--stdout'], {
      env: { ...process.env, LANG: 'C' },
    });
    ...
    processoLive.on('close', code => {
      ...
      ultimoErro = erroSaida.trim().split('\n').filter(Boolean).pop() || `gphoto2 saiu com código ${code}`;
    ...
    mudarEstado(ESTADO.LIVE_VIEW, { modelo });
  }
  ...
  async function religar() {
    ...
    modelo = await detectar();

    if (!modelo) {
      ultimoErro = 'nenhuma câmera detectada';
  ...
  async function capturar({ timeoutMs = 45_000 } = {}) {
    ...
    if (!modelo) throw new Error('Nenhuma câmera conectada');
    ...
    try {
      ...
      if (!gerados.length) throw new Error('A câmera disparou mas nenhum arquivo foi baixado');
