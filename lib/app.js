    ...
    try {
      ...
      const result = await handleUpload({
        ...
        onBeforeGenerateToken: async (pathname, clientPayload) => {
          ...
          if (!code) throw new Error(`Código de sessão ausente ou malformado (recebido: ${clientPayload ?? 'nada'})`);

          if (!(await store.has(code))) {
            // A causa quase sempre é esta: sem Redis, cada instância da
            // Vercel tem o próprio mapa em memória, e a sessão criada
            // numa não existe na outra que atende este upload.
            const hint = config.stateDriver === 'memory' && config.isVercel
              ? ' — o estado está em memória numa função serverless: conecte um Redis ao projeto (veja /api/health)'
              : '';
            throw new Error(`Sessão ${code} não encontrada${hint}`);
          }
