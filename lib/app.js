...
const PHOTO_ID_RE = /^\d+_\d+x\d+_[0-9a-f]{8}$/;
...
function clientConfig() {
  return {
    ...
    preview: {
      ...
      // O frame curto fica disponível para diagnóstico e clientes que
      // não entendem MJPEG.
      framePath: config.camera.preview === 'gphoto'
        ? `/api/preview/${config.camera.previewCode}/frame`
        : null,
      // O app abre um único stream por tela. Não trocar o src a cada
      // quadro é o que mantém a imagem contínua e sem clarões.
...
async function createApp() {
  ...
  const streamsPreviewPorViewer = new Map();
  ...
  const camera = config.camera.fonte === 'gphoto'
    ...
    : null;
  const capturasPorRequisicao = new Map();
  ...
  /* A nuvem NÃO é o totem.
     As telas de operação — telão, controle, câmera de navegador — só
     fazem sentido no computador que tem a câmera cabeada. Servi-las na
     Vercel fazia o convidado cair numa página pedindo webcam e
     pareamento por QR, coisas que a arquitetura com DSLR aposentou.
     Lá o app tem um papel só: entregar a foto a quem escaneou o QR. */
  if (config.isVercel) {
    const SO_DO_TOTEM = /^\/(totem|display|control|camera|index)\.html$|^\/$/;
    app.get(SO_DO_TOTEM, (req, res) => {
      res.set('Content-Type', 'text/html; charset=utf-8');
      res.set('Cache-Control', 'public, max-age=300');
      res.send(paginaDaNuvem());
    });
  }
  ...
  function photoUrlsFromRecord(photo) {
    ...
    return {
      ...
      ts: photo.capturedAt,
      // A galeria remonta o QR a partir daqui: sem a URL pública ela
      // ofereceria o endereço da LAN mesmo depois de a foto ter subido.
  ...
  async function persistPhoto(refs, meta, session) {
    ...
    if (repo) {
      ...
      shareQueue?.tick().catch(err => {
        logStructured('error', 'fila de publicação não iniciou', { captureId: id, error: err.message });
      });
    } else {
  ...
  app.get('/api/health', async (req, res) => {
    ...
    if (shareQueue) {
      ...
      // Na Vercel não existe fila local: ela recebe arquivos já
      // publicados pelo Mac e só precisa informar se o destino existe.
      health.cloud = publisher.configured ? 'ready' : 'not-configured';
    }
  ...
  app.get('/api/preview/:code/stream', (req, res) => {
    ...
    const viewer = String(req.query.viewer || '').replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 80);
    const viewerKey = viewer ? `${code}:${viewer}` : null;
    const fluxo = { res, cancelar: () => {} };
    if (viewerKey) {
      const anterior = streamsPreviewPorViewer.get(viewerKey);
      if (anterior && anterior.res !== res) {
        anterior.cancelar();
        anterior.res.destroy();
      }
      streamsPreviewPorViewer.set(viewerKey, fluxo);
    }
    ...
    let aguardandoDrain = false;
    let bloqueadoDesde = 0;

    fluxo.cancelar = preview.assinar(
      code,
      jpeg => {
        if (res.destroyed || res.writableEnded) return;

        /* Cada tela tem sua própria velocidade. Se o WKWebView do
           telão parar de consumir, `write()` devolve false. Continuar
           empilhando quadros faria essa tela ficar minutos atrasada
           enquanto o controle — outra conexão — segue ao vivo. */
        if (aguardandoDrain) {
          if (Date.now() - bloqueadoDesde > 2_000) res.destroy();
          return; // quadro atrasado não tem valor
        }

        if (!res.write(quadroMjpeg(jpeg))) {
          aguardandoDrain = true;
          bloqueadoDesde = Date.now();
          res.once('drain', () => {
            aguardandoDrain = false;
            bloqueadoDesde = 0;
          });
        }
      },
    ...
    const limpar = () => {
      ...
      encerrado = true;
      fluxo.cancelar();
      if (viewerKey && streamsPreviewPorViewer.get(viewerKey) === fluxo) {
        streamsPreviewPorViewer.delete(viewerKey);
      }
    };
  ...
  /** Quadro avulso para diagnóstico e clientes sem suporte a MJPEG. */
  app.get('/api/preview/:code/frame', (req, res) => {
    const code = normalizeCode(req.params.code);
    if (!code) return res.status(400).end();

    const quadro = preview.quadroAtual(code);
    res.set({
      'Cache-Control': 'no-store, no-cache, must-revalidate, private',
      Pragma: 'no-cache',
      'X-Accel-Buffering': 'no',
    });
    if (!quadro) return res.status(503).end();

    res.set({
      'Content-Type': 'image/jpeg',
      'Content-Length': String(quadro.jpeg.length),
      'X-Preview-Sequence': String(quadro.sequencia),
      'X-Preview-Age-Ms': String(Math.max(0, Date.now() - quadro.recebidoEm)),
    });
    res.end(quadro.jpeg);
  });
   ...
   *
   * O telão roda em localhost, então `req.get('host')` devolve
   * "localhost:3000" — e um convidado que escaneasse isso tentaria
   * abrir a foto no próprio celular. É o defeito mais silencioso
   * possível: o QR aparece bonito na tela e simplesmente não funciona
   * para ninguém.
   *
   * A ordem reflete quem consegue abrir o link:
   *
   *   1. nuvem  — funciona de qualquer lugar, semanas depois
   *   2. LAN    — funciona para quem está no Wi-Fi do evento
   *
   * A nuvem só entra aqui se estiver configurada. Quando a publicação
   * termina, o telão troca o QR pela URL definitiva sozinho, pelo
   * evento share-status — então o convidado nunca fica sem um QR
   * válido, mesmo enquanto o upload acontece.
   */
  function enderecoPublicoDaFoto({ host = '', protocol = 'http' } = {}, pageUrl) {
    // URL pública sem credencial de upload gera um QR que fica esperando
    // para sempre. Os dois lados precisam estar configurados juntos.
    if (publisher.configured && config.publicBaseUrl) {
      return `${String(config.publicBaseUrl).replace(/\/+$/, '')}${pageUrl}`;
    }

    const ehLocal = /^(localhost|127\.0\.0\.1|\[::1\])(:|$)/i.test(host);
    if (!ehLocal) return `${protocol}://${host}${pageUrl}`;

    const lan = primaryLanAddress();
    return lan
      ? `http://${lan}:${config.port}${pageUrl}`
      : `${protocol}://${host}${pageUrl}`; // sem rede: ao menos abre no próprio totem
  }
  ...
  app.post('/api/camera/prepare', async (req, res) => {
    if (!camera) return res.status(400).json({ error: 'Nenhuma câmera configurada' });
    try {
      const session = await loadSession(req.body?.code || config.camera.previewCode);
      const flash = session?.settings?.flashMode === 'flash';
      res.json({ success: true, ...(await camera.prepararCaptura({ flash })) });
    } catch (err) {
  ...
  /**
   * Controla o flash embutido sem fingir uma capacidade que a câmera não
   * tem. A Canon permite levantá-lo, mas recolher continua sendo manual.
   * A preferência fica na sessão e é reaplicada no preparo de cada foto.
   */
  async function ajustarFlashInterno(code, mode) {
    if (!['off', 'flash'].includes(mode)) {
      const erro = new Error('Modo de flash inválido');
      erro.statusCode = 400;
      throw erro;
    }

    const session = await loadSession(code || config.camera.previewCode);
    if (!session) {
      const erro = new Error('Sessão não encontrada');
      erro.statusCode = 404;
      throw erro;
    }

    const modoAnterior = session.settings.flashMode;
    let flash = camera?.status().flash || null;
    if (mode === 'flash') {
      if (!camera) {
        const erro = new Error('Nenhuma câmera configurada');
        erro.statusCode = 400;
        throw erro;
      }
      flash = await camera.levantarFlash();
    }

    if (modoAnterior !== mode) {
      session.settings.flashMode = mode;
      await store.put(session);
    ...
    return {
      ...
      mode,
      flash,
      requerFechamentoManual: mode === 'off' && modoAnterior === 'flash' && flash?.podeRecolher === false,
    };
  }

  app.post('/api/camera/flash', async (req, res) => {
    try {
      res.json(await ajustarFlashInterno(req.body?.code, req.body?.mode));
    } catch (err) {
   ...
   */
  async function executarCapturaDslr({ body = {}, host = '', protocol = 'http' } = {}) {
    const inicio = Date.now();
    let arquivo = null;

    try {
      const code = body.code || config.camera.previewCode;
      const session = await loadSession(code);
      const usarFlash = session?.settings?.flashMode === 'flash';
      const disparo = await camera.capturar({
        timeoutMs: config.camera.capturaTimeoutMs,
        flash: usarFlash,
      });
      ...
      const alvo = enderecoPublicoDaFoto({ host, protocol }, dados.pageUrl);
  ...
  function idDeCaptura(value) {
    return /^[a-f0-9-]{20,50}$/i.test(String(value || '')) ? String(value) : null;
  }

  function tarefaDeCaptura(contexto, origem) {
    const requestId = idDeCaptura(contexto.body?.requestId);
    let tarefa = requestId ? capturasPorRequisicao.get(requestId) : null;
    const reutilizada = !!tarefa;

    if (!tarefa) {
      logStructured('info', 'captura_solicitada', { requestId, origem });
      tarefa = executarCapturaDslr(contexto);
      if (requestId) {
        capturasPorRequisicao.set(requestId, tarefa);
        const expirar = () => setTimeout(() => capturasPorRequisicao.delete(requestId), 60_000).unref?.();
        tarefa.then(expirar, expirar);
      }
    } else {
      logStructured('info', 'captura_reutilizada', { requestId, origem });
    }

    return { requestId, tarefa, reutilizada };
  }
  ...
  app.post('/api/capture', async (req, res) => {
    ...
    const inicio = Date.now();
    const { requestId, tarefa } = tarefaDeCaptura({
      body: req.body || {},
      host: req.get('host') || '',
    ...
    try {
      res.json(await tarefa);
    } catch (err) {
      logStructured('error', 'captura_falhou', { requestId, error: err.message, ms: Date.now() - inicio });
      res.status(500).json({ error: err.message });
    }
  });
  ...
  /**
   * Onde a foto entra dentro da moldura, em fração do quadro.
   *
   * É o que permite ao telão sobrepor a arte do evento ao preview ao
   * vivo com a MESMA geometria da composição final. Sem isso o convidado
   * se posiciona olhando um retângulo cheio e recebe uma foto recortada
   * noutro lugar — o defeito que a janela recortada existe para evitar.
   *
   * Frações, e não pixels: o telão escala à vontade e a proporção
   * continua valendo.
   */
  /* Cache por proporção: com duas artes no projeto, guardar uma
     geometria só faria o telão mostrar a janela da orientação errada
     depois que o operador girasse. */
  const geometriaEmCache = new Map();

  app.get('/api/frame/:code/geometria', async (req, res) => {
    const session = await loadSession(req.params.code);
    const ratio = req.query.ratio === '4:3' ? '4:3' : '3:4';

    // A moldura padrão não muda durante o evento; medir o alpha de uma
    // arte grande a cada consulta seria desperdício num loop de telão.
    const daSessao = session && frameFor(session, ratio);
    if (!daSessao && geometriaEmCache.has(ratio)) {
      return res.json(geometriaEmCache.get(ratio) || { temMoldura: false });
    }

    try {
      const { loadFrame, detectarJanela } = require('./photo');
      const buffer = await loadFrame(withFrames(session, ratio), ratio);
      if (!buffer) {
        if (!daSessao) geometriaEmCache.set(ratio, null);
        return res.json({ temMoldura: false });
      }

      const janela = await detectarJanela(buffer);
      const resposta = janela ? {
        temMoldura: true,
        moldura: { largura: janela.frameWidth, altura: janela.frameHeight },
        janela: {
          esquerda: +(janela.left / janela.frameWidth).toFixed(5),
          topo: +(janela.top / janela.frameHeight).toFixed(5),
          largura: +(janela.width / janela.frameWidth).toFixed(5),
          altura: +(janela.height / janela.frameHeight).toFixed(5),
        },
        frameUrl: `/api/frame/${session?.code || config.camera.previewCode}?ratio=${encodeURIComponent(ratio)}`,
      } : { temMoldura: true, janela: null };

      if (!daSessao) geometriaEmCache.set(ratio, resposta);
      res.json(resposta);
    } catch (err) {
      logStructured('warn', 'geometria da moldura indisponível', { error: err.message });
      res.json({ temMoldura: false });
    }
  });
  ...
  app.get('/api/frame/:code', async (req, res) => {
    ...
    const ratio = req.query.ratio === '4:3' ? '4:3' : '3:4';
    const frame = frameFor(session, ratio);

    /* Sem moldura na sessão, serve a padrão do projeto — a MESMA que a
       composição usa. Antes esta rota só conhecia o banco: o telão e o
       painel mostravam "sem moldura" enquanto as fotos saíam com ela,
       e ninguém conseguia conferir o enquadramento antes de disparar. */
    if (!frame) {
      const padrao = await require('./photo').loadFrame(null, ratio);
      if (!padrao) return res.status(404).send('Sem moldura');

      res.set('Content-Type', 'image/png');
      res.set('Cache-Control', 'no-store');
      return res.send(padrao);
    }
  ...
  app.get('/photo/:id', async (req, res) => {
    ...
    if (!record) {
      const id = safeBasename(req.params.id).replace(/\.jpg$/i, '');
      if (config.isVercel && PHOTO_ID_RE.test(id)) {
        res.set('Content-Type', 'text/html; charset=utf-8');
        res.set('Cache-Control', 'no-store');
        return res.status(202).send(renderPendingPhotoPage(id));
      }
  ...
  io.on('connection', socket => {
    ...
    socket.on('trigger-capture', async ({ code, timer } = {}) => {
      const session = await loadSession(code);
      if (!session) return;
      io.to(roomFor(session.code, 'display')).emit('start-countdown', {
        timer: timer || session.settings.timer,
        requestId: randomUUID(),
      });
    });

    /* O WKWebView pode deixar um POST novo esperando atrás do stream
       MJPEG. O socket já está conectado — foi ele que recebeu a ordem de
       contagem — e por isso é o canal confiável para iniciar o disparo.
       O requestId mantém HTTP e socket idempotentes no fallback. */
    socket.on('dslr-capture', async (payload = {}, callback = () => {}) => {
      if (!camera) return callback({ success: false, error: 'Nenhuma câmera configurada' });
      if (socket.data.role !== 'display') return callback({ success: false, error: 'Somente o telão pode disparar' });

      const code = normalizeCode(socket.data.code || payload.code);
      if (!code) return callback({ success: false, error: 'Sessão inválida' });

      const host = socket.handshake.headers.host || `localhost:${config.port}`;
      const protocol = String(socket.handshake.headers['x-forwarded-proto'] || 'http').split(',')[0].trim();
      const contexto = { body: { ...payload, code }, host, protocol };
      const { requestId, tarefa, reutilizada } = tarefaDeCaptura(contexto, 'socket');

      try {
        const resultado = await tarefa;
        if (!reutilizada) {
          io.to(roomFor(code, 'display')).emit('dslr-capture-result', { requestId, resultado });
        }
        callback(resultado);
      } catch (err) {
        const falha = { success: false, error: err.message };
        if (!reutilizada) {
          logStructured('error', 'captura_falhou', { requestId, origem: 'socket', error: err.message });
          io.to(roomFor(code, 'display')).emit('dslr-capture-result', { requestId, resultado: falha });
        }
        callback(falha);
      }
    });

    socket.on('camera-flash', async (payload = {}, callback = () => {}) => {
      if (socket.data.role !== 'control') {
        return callback({ success: false, error: 'Somente o controle pode ajustar o flash' });
      }

      try {
        const code = normalizeCode(socket.data.code || payload.code);
        callback(await ajustarFlashInterno(code, payload.mode));
      } catch (err) {
    ...
    socket.on('show-photo', async ({ code, photoId } = {}) => {
      const session = await loadSession(code);
      if (!session || !photoId) return;

      const photo = listPhotos(session).find(item => item.page?.endsWith(`/${photoId}`));
      if (photo) io.to(roomFor(session.code, 'display')).emit('show-photo', { photo });
    });
    ...
    socket.on('display-state', ({ code, state } = {}) => {
      const session = normalizeCode(code);
      if (!session || socket.data.role !== 'display' || !state) return;
      io.to(roomFor(session, 'control')).emit('display-state', { state });
    });
 ...
 * A raiz na nuvem.
 *
 * Quem chega aqui sem um id de foto ou errou o link, ou está curioso.
 * Nos dois casos a resposta honesta é dizer o que este endereço faz —
 * e não pedir webcam nem oferecer pareamento, que é o que as telas do
 * totem faziam quando eram servidas por engano.
 */
function paginaDaNuvem() {
  return `<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Globo Photo Booth</title>
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  h1{font-size:clamp(28px,6vw,55px);font-weight:700;line-height:1;margin-bottom:16px}
  p{font-size:clamp(16px,2.5vw,22px);line-height:1.45;color:#939598;max-width:30ch;margin:0 auto}
...
<body>
  <main>
    <h1>Globo Photo Booth</h1>
    <p>Este endereço entrega as fotos do evento. Escaneie o QR Code que aparece no totem para abrir a sua.</p>
  </main>
  <div class="barra"></div>
</body>
</html>`;
...
function renderPendingPhotoPage(id) {
  const path = `/photo/${encodeURIComponent(id)}`;
  return `<!DOCTYPE html>
<html lang="pt-BR">
<head>
  <meta charset="UTF-8">
  ...
  <title>Publicando sua foto — Globo</title>
  <style>
    * { box-sizing: border-box; }
    body { margin: 0; min-height: 100vh; font-family: Inter, system-ui, -apple-system, "Segoe UI", sans-serif; background: #06121f; color: #fff; display: grid; place-items: center; padding: 24px; text-align: center; }
    main { width: min(100%, 440px); display: flex; flex-direction: column; align-items: center; gap: 18px; }
    .loader { width: 58px; height: 58px; border: 6px solid rgba(255,255,255,.2); border-top-color: #fff; border-radius: 50%; animation: giro .9s linear infinite; }
...
<body>
  <main>
    ...
    <h1>Sua foto está chegando</h1>
    <p>O upload começou no totem. Esta página atualiza sozinha e libera o download assim que terminar.</p>
    <a href="${path}">Atualizar agora</a>
  </main>
</body>
</html>`;
}
