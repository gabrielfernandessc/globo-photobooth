...
async function createApp() {
   ...
   */
  function enderecoPublicoDaFoto(req, pageUrl) {
    ...
    if (!ehLocal) return `${req.protocol}://${host}${pageUrl}`;

    const lan = primaryLanAddress();
    return lan
      ? `http://${lan}:${config.port}${pageUrl}`
      : `${req.protocol}://${host}${pageUrl}`; // sem rede: ao menos abre no próprio totem
  }
  ...
  app.post('/api/capture', async (req, res) => {
    ...
    try {
      ...
      const alvo = enderecoPublicoDaFoto(req, dados.pageUrl);
