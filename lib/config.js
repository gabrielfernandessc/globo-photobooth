...
const config = {
  ...
  /* As fotos moram DENTRO do DATA_DIR, junto do banco.
     Antes caíam em public/uploads enquanto o banco ficava em data/ — o
     comentário abaixo prometia "copiar a pasta do evento leva tudo" e
     era mentira: levaria o banco sem as fotos. Pior, ficavam servidas
     como estático dentro do diretório público. */
  uploadsDir: process.env.UPLOADS_DIR || path.join(DATA_DIR, 'uploads'),

  /* ── Câmera ──
     A FOTO vem sempre da DSLR por USB. O que varia é de onde vem o
     PREVIEW, e essa escolha é de risco, não de gosto:

     gphoto      live view pelo mesmo cabo USB. Um cabo só, mas depende
                 de o driver entregar quadros suficientes — o que varia
                 muito entre Canon e Sony.
     capturadora HDMI limpo da câmera para uma placa de captura, que
                 aparece como webcam. O telão lê por getUserMedia em
                 localhost, que é contexto seguro SEM certificado.
     nenhum      sem preview ao vivo. O telão conduz a contagem e mostra
                 a foto com o QR depois do disparo.

     "nenhum" não é o modo degradado: é o mais confiável dos três,
     porque remove a única peça que pode travar durante o evento sem
     que ninguém perceba. Um totem sem preview ainda entrega a foto;
     um preview travado faz o operador achar que tudo parou. */
  camera: {
    // A função pública só visualiza arquivos do Blob. Tentar iniciar
    // gphoto ali escreve em /var/task (read-only) e derruba até a página
    // do QR com 503 antes de qualquer rota responder.
    fonte: isVercel ? 'nenhum' : (process.env.CAMERA_SOURCE || 'gphoto'),
    preview: isVercel ? 'nenhum' : (process.env.PREVIEW_SOURCE || 'gphoto'),
    binario: process.env.GPHOTO_BIN || 'gphoto2',
    capturaTimeoutMs: int(process.env.CAPTURE_TIMEOUT_MS, 45_000),
    /* Sessão do preview da câmera cabeada.
       Com DSLR há exatamente uma câmera e um telão, no mesmo
       computador — não existe pareamento a fazer, então o código é fixo
       e conhecido pelos dois lados. */
    previewCode: 'TOTM',
  },

  /* Segunda cópia do original, em OUTRO disco.
     Com a câmera em "PC Only" o cartão sai do caminho crítico do
     disparo — e some junto a única cópia que não estava no Mac. Uma
     pasta num HD externo devolve essa redundância sem colocar nada de
     volta no meio da captura. Vazio = sem cópia extra. */
  backupDir: process.env.BACKUP_DIR || null,

  /* Moldura padrão do evento, versionada com o projeto.
     Carregar moldura não deveria ser ritual de véspera: ela é parte da
     identidade do evento, não um dado de sessão. Este arquivo vale
     sempre que a sessão não tiver uma moldura própria — então sobrevive
     a limpeza do data/, a restart e a troca de máquina.
     O upload pelo painel continua existindo, e tem prioridade. */
  /* Uma moldura por proporção, escolhida pelo arquivo.
     O operador troca a orientação no painel e a arte acompanha — no
     preview e na foto — sem upload nenhum. Basta o arquivo existir.

       3:4 (em pé)     frames/vertical.png
       4:3 (deitado)   frames/horizontal.png

     padrao.png continua valendo para a proporção que não tiver arquivo
     próprio, então um totem com uma arte só segue funcionando. */
  framesDir: process.env.FRAMES_DIR || path.join(__dirname, '..', 'frames'),
  defaultFramePath: process.env.DEFAULT_FRAME || null,
