...
function createPreviewHub({ log = () => {} } = {}) {
   ...
   */
  function assinar(code, escrever) {
    const s = estado(code);
    s.assinantes.add(escrever);
    if (s.ultimoQuadro) escrever(s.ultimoQuadro);
    return () => s.assinantes.delete(escrever);
  }
  ...
  return { publicar, assinar, status, temAudiencia, encerrar, FRONTEIRA };
}
