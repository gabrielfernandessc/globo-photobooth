/* ══════════════════════════════════════════════════════════
   ...
   Aqui o celular faz POST de quadros JPEG e o servidor guarda somente
   o mais recente. Cada tela abre um único stream MJPEG e o navegador
   decodifica a sequência sem trocar o elemento visual entre quadros.
...
function createPreviewHub({ log = () => {} } = {}) {
  ...
  /** Entrega uma fotografia imutável do quadro atual para respostas curtas. */
  function quadroAtual(code) {
    const s = sessoes.get(code);
    if (!s?.ultimoQuadro) return null;
  ...
  return { publicar, assinar, status, quadroAtual, temAudiencia, encerrar, encerrarTodos, FRONTEIRA };
}
